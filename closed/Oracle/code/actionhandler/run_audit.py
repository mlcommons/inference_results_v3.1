...
class RunAuditHandler(RunHarnessHandler):
    ...
    def is_exempt(self):
        if self.test_name == AuditTest.TEST01:
            exclude_list = [
                Benchmark.GPTJ, Benchmark.GPT175
            ]
            if self.benchmark_conf["benchmark"] in exclude_list:
                return True
        if self.test_name == AuditTest.TEST04:
            exclude_list = [
                Benchmark.BERT, Benchmark.DLRMv2, Benchmark.RNNT,
                Benchmark.UNET3D, Benchmark.Retinanet, Benchmark.GPTJ, Benchmark.GPT175
            ]
            if self.benchmark_conf["benchmark"] in exclude_list:
                return True
        if self.test_name == AuditTest.TEST05:
            exclude_list = [Benchmark.GPTJ, Benchmark.GPT175]
            if self.benchmark_conf["benchmark"] in exclude_list:
                return True
        return False
