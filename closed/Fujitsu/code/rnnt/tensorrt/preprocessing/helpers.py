...
def print_once(msg):
    if torch.distributed.is_available() and (not torch.distributed.is_initialized() or (torch.distributed.is_initialized() and torch.distributed.get_rank() == 0)):
        print(msg)
...
def process_evaluation_epoch(global_vars: dict, tag=None):
    ...
    multi_gpu = torch.distributed.is_available() and torch.distributed.is_initialized()
    if multi_gpu:
