...
public:
    ...
    void getFreeBufIndexes(std::vector<size_t>& bufIndex)
    {
        ...
        NVTX_GLOBAL_START_WITH_PAYLOAD(
            nvtxDaliWaitForBufSpace, "Dali::WaitForFreeBufIndexes", nvtx::COLOR_YELLOW_4, mRequestedDaliBatchSize);
        while (!(mBufState.mNumBufLines - mBufState.mReservedIndexes >= mRequestedDaliBatchSize))
        {
        };
        NVTX_GLOBAL_END(nvtxDaliWaitForBufSpace);
    ...
    void releaseBatch(std::vector<size_t>& bufIndex)
    {
        ...
        NVTX_GLOBAL_START_WITH_PAYLOAD(nvtxReleaseBatch, "DALI::ReleaseBatch", nvtx::COLOR_YELLOW_2, bufIndex.size());
        std::unique_lock<std::mutex> lock(mBufState.mMutex);
        ...
        NVTX_GLOBAL_END(nvtxReleaseBatch);
    ...
    void getBatch(std::vector<sampleAttributes>& batch, size_t maxBatchSize)
    {
        ...
        NVTX_GLOBAL_START_WITH_PAYLOAD(nvtxGetEncoderBatch, "DALI::GetBatch", nvtx::COLOR_YELLOW_3, samplesInThisBatch);
        ...
        NVTX_GLOBAL_END(nvtxGetEncoderBatch);
    }
        ...
        size_t deviceIdx)
    {
        ...
        {
            NVTX_GLOBAL_START_WITH_PAYLOAD(
                nvtxMakeDaliBatch, "DALI:makeDaliBatch", nvtx::COLOR_YELLOW_0, samples.size());
        ...
        NVTX_GLOBAL_END(nvtxMakeDaliBatch);
        return true;
    }
};
    ...
    void processDaliBatches()
    {
        ...
        while (!mDone)
        {
            ...
            for (auto bufId = 0; bufId < mDaliPipelineDepth; bufId++)
            {
                ...
                {
                    NVTX_GLOBAL_START(nvtxExecuteDaliBatch, "DALI:RunDaliBatch", nvtx::COLOR_YELLOW_1);
                    ...
                    NVTX_GLOBAL_END(nvtxExecuteDaliBatch);
                }
            }
            for (auto bufId = 0; bufId < mDaliPipelineDepth; bufId++)
            {
                if (batchIssued[bufId] == true)
                {
                    NVTX_GLOBAL_START(nvtxallocateDaliBatch, "DALI:allocateDaliBatch", nvtx::COLOR_GREEN_3);
                    mAudioBufManager->allocateDaliBatch(
                        mD2DStream, bufId, bufIndex[bufId], mScatterGatherD2DData, mScatterGatherD2DSeqLen);
                    NVTX_GLOBAL_END(nvtxallocateDaliBatch);
                }
            }
        }
    }
