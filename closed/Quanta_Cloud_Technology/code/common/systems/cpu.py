...
class CPU(Hardware):
    ...
    def detect(cls) -> CPU:
        ...
        arch = cpu_fields.get("Architecture", "UnknownArch")
        model_id = cpu_fields.get("Model", "UnknownNum")
        return CPU(
            cpu_fields.get("Model name", arch+"_"+model_id),
            CPUArchitecture(arch),
            int(cpu_fields.get("Core(s) per socket", cpu_fields.get("Core(s) per cluster", "0"))),
            int(cpu_fields.get("Thread(s) per core", "1")))
...
class CPUConfiguration(Hardware):
    ...
    def detect(cls) -> CPUConfiguration:
        ...
        for i, cpu_fields in enumerate(CPU_INFO_SOURCE):
            count = int(cpu_fields.get("Socket(s)", 1).replace("-", "1"))
