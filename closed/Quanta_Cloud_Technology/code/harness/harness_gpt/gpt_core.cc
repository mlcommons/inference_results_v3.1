...
extern void launch_top_kernel(const void* input, void* output, int32_t bs, int32_t vocab_size, cudaStream_t stream);
...
// std::vector<BERTGraphSpec_t> GPTCore::ParseGraphSpecs(const std::string& graphSpecs, int graphSeqLenUpperBound)
// {

// }

// void GPTCore::InitializeGraphSpecs(int graphMaxSeqLen, const std::string& graphSpecs, int numBERTCores)
// {

// }

// void GPTCore::BuildGraphs(int graphMaxSeqLen, const std::string& graphSpecs, int numBERTCores)
// {
    ...
    GPTManagedBuffer& gptBuffers, CopyStream& copyStream)
{
    ...
    int32_t const finalSeqLength{maxInputLength + step};
    ...
    if (isBeamSearch)
    {
        ...
        auto workspace
            = mDecoderBufferManager->gpu(actualBatchSize * beamWidth * finalSeqLength, nvinfer1::DataType::kINT32);
        mDecoderBufferManager->setZero(*workspace);

        // For sampling, it is equivalent to all parent ids are 0.
        tensorrt_llm::kernels::gatherTreeParam param;
        param.beams = tr::bufferCast<int32_t>(*workspace);
        // Remove prompt length if possible
        param.max_sequence_lengths = tr::bufferCast<int32_t>(*(mDecoderOutput->lengths));
        ...
        param.batch_size = actualBatchSize;
        param.beam_width = beamWidth;
        param.step_ids = tr::bufferCast<int32_t>(*(mDecoderOutput->ids));
        param.parent_ids = tr::bufferCast<int32_t>(*(mDecoderOutput->parentIds));
        param.end_tokens = tr::bufferCast<int32_t>(*(mDecoderInput->endIds));
        param.max_input_length = maxInputLength;
        param.input_lengths = tr::bufferCast<int32_t>(*(mDecoderInput->lengths));
        ...
        param.output_ids = tr::bufferCast<int32_t>(*gatheredOutputIds);
        param.stream = mInferStream;
    ...
    for (int32_t b = 0; b < actualBatchSize; ++b)
    {
        int32_t const realInputLength = static_cast<int32_t*>(gptBuffers.getHostBuffer(mGPTModel->mInputLengthName))[b];
        for (int32_t id = 0; id < step && id < kGPT_MAX_OUTPUT_LENGTH; ++id)
        {
            ...
            // [batch_max_length + id, b, beamWidth] for greedy
            // [b, 0, batch_max_length + id] for beam search
            size_t const srcOffset = isBeamSearch
                ? tc::flat_index3(b, 0, realInputLength + id, beamWidth, finalSeqLength)
                : tc::flat_index3(maxInputLength + id, b, 0, actualBatchSize, mGPTModel->mBeamWidth);
            ;
            CHECK_EQ(
                cudaMemcpyAsync(static_cast<char*>(gptBuffers.getHostOutputTokenBuffer(b)) + dstOffset * elementSize,
...
{
    // TRT LLM July Release's engine only has 1 optimization profile
...
void GPTCore::warmUp()
{
    ...
    return;
    CHECK_EQ(cudaSetDevice(mDeviceId), cudaSuccess);
    LOG(INFO) << "Device " << mDeviceId << ": Warming up GPTCore.";
    // dummy warmup on context phase + copystream 0
    int32_t warmUpCopyStreamIdx{0};
    auto& gptBuffers = mGPTBufferVec[warmUpCopyStreamIdx];
    gptBuffers.memsetDeviceAll(0);
    int32_t maxInputLength = mGPTModel->mMaxInputLength;
    auto context = getContext(maxInputLength, /*isContextPhase = */ true);
    mGPTModel->setInputShapes(context, mMaxBatchSize, maxInputLength, /*step = */ 0, /*isContext = */ true);
    mGPTModel->setTensorAddresses(context, gptBuffers, /*step = */ 0);

    CHECK_EQ(context->enqueueV3(mInferStream), true);
    CHECK_EQ(cudaStreamSynchronize(mInferStream), cudaSuccess);
    gptBuffers.memsetDeviceAll(0);

    LOG(INFO) << "Device " << mDeviceId << ": Finished warming up GPTCore.";
}
