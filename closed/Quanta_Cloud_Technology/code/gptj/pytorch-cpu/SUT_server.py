...
class Consumer(mp.Process):
    def __init__(self, model_checkpoint_path="", precision="int8", quantized_model="", dataset_path="", input_queue=None, out_queue=None, lock=None, cond_var=None, init_counter=None, proc_idx=None, start_core_idx=0, cpus_per_proc=56, workers_per_proc=1, warmup=False, total_sample_count=1000, pad_inputs=False):
...
class SUT(object):
    ...
    def startSUT(self):
        ...
        self.tracker_thread = threading.Thread(target=self.trackloadgenQueries)
    ...
    def createProcesses(self):
        ...
        for proc_idx in range(self.num_proc):
            self.procs[proc_idx] = Consumer(self.model_checkpoint_path, self.precision, self.quantized_model, self.dataset_path, self.input_queue, self.output_queue, self.lock, self.cv, self.init_counter, proc_idx, start_core, self.cpus_per_proc, self.workers_per_proc, warmup=self.warmup, total_sample_count = self.total_sample_count, pad_inputs=self.pad_inputs)
    ...
    def issueQueries(self, query_samples):
        ...
        self.ids.append(query_samples[0].id)
        index = query_samples[0].index
        self.indexes.append(index)
        if len(self.ids)==self.batch_size:
            item = InputItem(self.ids, self.indexes)
    ...
    def trackloadgenQueries(self):
        while True:
            if self.get_arrival_time() > 0 and (time.time() - self.get_arrival_time() > 20):
                if (len(self.ids) > 0):
                    item = InputItem(self.ids, self.indexes)
                    self.input_queue.put(item)
            if self.stopped:
                break
            if self.get_arrival_time() > 0 and (time.time() - self.get_arrival_time() > 60):
                break
            time.sleep(1)
