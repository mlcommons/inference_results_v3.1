...
int main(int argc, char **argv) {
  ...
  opts.add_options()
    ...
    ("n,num_numa", "Number of numa node",
  ...
  if (test_scenario == "Offline") {
    BertOfflineSUT sut(
        ...
        intra_parallel, num_numa, batch_size, watermark, warmup, profiler_flag, profiler_folder);
    ...
    BertServerSUT server_sut(
        model_file, sample_file, inter_parallel,
        intra_parallel, num_numa, batch_size, watermark, upper_watermark, warmup, profiler_flag, profiler_folder);
