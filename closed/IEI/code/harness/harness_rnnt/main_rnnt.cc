...
void Stream::processBatches()
{
    ...
    while (!mDone)
    {
        ...
        // run encoder
        NVTX_GLOBAL_START(nvtxRunEnc, "ENC:run", nvtx::COLOR_BLUE_0);
        frontEndBatch(p);
        NVTX_GLOBAL_END(nvtxRunEnc);
        ...
        // run decoder portion
        NVTX_GLOBAL_START(nvtxRunDec, "DEC: run", nvtx::COLOR_GREEN_0);
        backEndBatchPrologue(p);
        backEndBatch(p);
        NVTX_GLOBAL_END(nvtxRunDec);
...
void Stream::processBatchesFrontEnd()
{
    ...
    {
        // run encoder
        NVTX_GLOBAL_START(nvtxWaitFe, "ENC: wait for FE", nvtx::COLOR_PINK_0);
        do
        {
            avail_samples = frontEndBatchPrologue(p, availBatchSlots);
        } while (!avail_samples && !mDone);
        NVTX_GLOBAL_END(nvtxWaitFe);

        nvtxRangeId_t NVTX_GLOBAL_START_WITH_PAYLOAD(nvtxRunEnc, "ENC:run", nvtx::COLOR_BLUE_0, actualBatchSize[p]);
        if (avail_samples)
        {
            frontEndBatch(p);
        }
        ...
        NVTX_GLOBAL_START(nvtxWaitDecEng, "ENC: wait for decoder engine", nvtx::COLOR_RED_0);
        wait_for_thread(mDone, encoderCredits, mutex_b2f, cv_b2f, "threadSynch: processBatchesFrontEnd[0]: ", p, iCnt);
        NVTX_GLOBAL_END(nvtxWaitDecEng);

        // transfer the state to the decoder
        NVTX_GLOBAL_START(nvtxTransfer, "ENC: transfer state", nvtx::COLOR_BLUE_2);
        auto daliIdxsToClear = transfer_state_from_encoder_to_decoder(mEncoderState, mDecoderState);
        if (FLAGS_enable_audio_processing)
        {
            mAudioBufManager->releaseBatch(daliIdxsToClear);
        ...
        NVTX_GLOBAL_START(nvtxWaitDecInit, "ENC: wait for decoder init", nvtx::COLOR_RED_1);
        wait_for_thread(mDone, encoderCredits, mutex_b2f, cv_b2f, "threadSynch: processBatchesFrontEnd[1]: ", p, iCnt);
        NVTX_GLOBAL_END(nvtxWaitDecInit);
...
void Stream::processBatchesBackEnd()
{
    ...
    while (!mDone)
    {
        ...
        NVTX_GLOBAL_START(nvtxWaitEncTransfer, "DEC: wait for encoder transfer", nvtx::COLOR_RED_0);
        wait_for_thread(mDone, decoderCredits, mutex_f2b, cv_f2b, "threadSynch: processBatchesBackEnd: ", p, iCnt);
        NVTX_GLOBAL_END(nvtxWaitEncTransfer);

        // initialize tensors
        NVTX_GLOBAL_START(nvtxDecInit, "DEC: tensor init", nvtx::COLOR_GREEN_1);
        if (actualBatchSize[p])
        {
            backEndBatchPrologue(p);
        }
        NVTX_GLOBAL_END(nvtxDecInit);
        ...
        // run decoder portion
        nvtxRangeId_t NVTX_GLOBAL_START_WITH_PAYLOAD(nvtxRunDec, "DEC: run", nvtx::COLOR_GREEN_0, actualBatchSize[p]);
...
int main(int argc, char* argv[])
{
    ...
    // Scope to force all smart objects destruction before CUDA context resets
    {
        ...
        if (FLAGS_num_warmups)
        {
            ...
            NVTX_GLOBAL_START(nvtxWarmUp, "Main::Warmup", nvtx::COLOR_BLUE_3);
            rnnt_server->Warmup();
            gLogInfo << "Finished warming up SUT." << std::endl;
            NVTX_GLOBAL_END(nvtxWarmUp);
        }
