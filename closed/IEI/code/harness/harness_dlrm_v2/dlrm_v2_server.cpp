...
DLRMv2Server::DLRMv2Server(const std::string name, const std::string enginePath,
    ...
    uint64_t serverNumIssueQueryThreads, double elRatio, NumaToQSLMap numaToQslMap, bool verbostNVTX)
    : mName{name}
    , mQsls{qsls}
    ...
    , mVerboseNVTX(verbostNVTX)
{
    ...
    if (!startFromDevice)
    {
        int numBatchMakers = mServerNumIssueQueryThreads > 0 ? mServerNumIssueQueryThreads : 1;
...
void DLRMv2Server::ProcessTasks(std::shared_ptr<DLRMv2Core> dlrmv2Core, int deviceId, int profileIdx)
{
    ...
    while (true)
    {
        ...
        int batchMakerIdx = mServerNumIssueQueryThreads > 0 ? deviceId : 0;
