...
void Batch::ContiguityAwareH2H(size_t tasksOffset, size_t pairsOffset, size_t pairsToCopy)
{
    ...
    size_t numericInputVolume = mNumericInputBuf.ElemByteSize() / sizeof(DLRMv2NumericInputType);

    DLRMv2Task& firstTask = mTasks[tasksOffset];
    DLRMv2Task& lastTask = mTasks.back();

    // first address in the contiguous block starting from mTasks[0]
    DLRMv2NumericInputType* globalContigStart
    ...
    DLRMv2NumericInputType* expectedFirstTaskStart = globalContigStart + (pairsOffset * numericInputVolume);

    // first address of current subset of mTtasks
    DLRMv2NumericInputType* actualFirstTaskStart
    ...
    DLRMv2NumericInputType* expectedLastTaskStart = actualFirstTaskStart + (pairsToCopy * numericInputVolume);
    // starting address of last batch in this set of tasks
    DLRMv2NumericInputType* actualLastTaskStart
    ...
    if (!(externalContiguity && internalContiguity))
    {
        if (!mNumericInputBuf.isDefaultHostPtr())
        {
            ...
            auto catInpAddr = qsl->GetSampleAddress(mTasks[0].querySample.index, 1);
            mCategoricalInputBuf.H2H(
                static_cast<DLRMv2CategoricalInputType*>(catInpAddr),pairsOffset, 0);
        }
        ...
        if (internalContiguity)
        {
            ...
            auto catInpAddr = qsl->GetSampleAddress(firstTask.querySample.index, 1);
            mCategoricalInputBuf.H2H(
                static_cast<DLRMv2CategoricalInputType*>(catInpAddr), pairsToCopy, pairsOffset);
        }
        else
        {
...
void Batch::IndividualH2H(size_t tasksOffset, size_t pairsOffset, size_t pairsToCopy)
{
    ...
    while (copiedPairs < pairsToCopy)
    {
        ...
        auto numInpAddr = qsl->GetSampleAddress(qslIdx, 0);
        auto catInpAddr = qsl->GetSampleAddress(qslIdx, 1);

        mNumericInputBuf.H2H(static_cast<DLRMv2NumericInputType*>(numInpAddr), numIndividualPairs,
            pairsOffset + copiedPairs);
        mCategoricalInputBuf.H2H(static_cast<DLRMv2CategoricalInputType*>(catInpAddr),
...
BatchMaker::BatchMaker(size_t numStagingThreads, size_t numStagingBatches, size_t maxBatchSize,
    ...
    std::shared_ptr<DLRMv2SampleLibrary> qsl, int64_t deviceIdx, int64_t numaIdx, int64_t numaNum, 
    std::vector<int> cpus)
    : mMaxBatchSize(maxBatchSize)
