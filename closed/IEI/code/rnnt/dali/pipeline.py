...
class DALIInferencePipeline(Pipeline):
    ...
    def _dali_init_log(args: dict):
        if torch.distributed.is_available() and (not torch.distributed.is_initialized() or (torch.distributed.is_initialized() and torch.distributed.get_rank() == 0)):  # print once
