# MLPerf Inference v3.1 Optimized Implementations
...
IEI submits with multiple systems, each of which are in either the datacenter category, edge category, or both. In general, multi-GPU systems are submitted in datacenter, and single-GPU and single-MIG systems are submitted in edge.

Our submission implements several inference harnesses stored under closed/IEI/code/harness:
...
Benchmarks are stored in `closed/IEI/code`. Each benchmark, as per MLPerf Inference requirements, contains a `README.md` detailing instructions and documentation for that benchmark. **However**, as a rule of thumb, **follow this guide first** from start to finish before moving on to benchmark-specific `README`s, as this guide has many wrapper commands to automate the same steps across multiple benchmarks at the same time.
...
IEI's MLPerf Inference submission stores the models, datasets, and preprocessed datasets in a central location we refer to as a "Scratch Space".
...
- You, IEI, or MLCommons has decided that something in the preprocessing step needed to be altered
...
Enter the container by entering the `closed/IEI` directory and running:
...
IEI's submission preprocesses the datasets to prepare them for evaluation. These are operations like the following:
...
Note: the above steps won't work on Jetson-based system (e.g. Orin). Please preprocess the data on other cuda-enabled devices, and copy over the $(MLPERF_SCRATCH_PATH)/preprocessed_data directory if needed. If any data preprocessing make target fails, please try to run it inside the container following **Launching the environment** section below.
...
- The docker container does not copy the files, and instead **mounts** the working directory (closed/IEI) under /work in the container. This means you can edit files outside the container, and the changes will be reflected inside as well.
...
class A30X4_CUSTOM(OfflineGPUBaseConfig):
    ...
    # Please see IEI's submission config files for example values and which fields to keep.
...
```
Alternatively, **if your system uses a GPU that is already supported by IEI's MLPerf Inference Submission**, you can simply extend one of IEI's configs and override some values. In this case, our A30x4 config can extend A30x1 instead of `OfflineGPUBaseConfig`, and just redefine the `system` and `offline_expected_qps` fields:
...
class A30X4_CUSTOM(A30x1):
    ...
    offline_expected_qps = 1971.9999999999998 * 4  # Here, I add *4 since I copied the original QPS from the A30x1 config.
```
You can see the GPUs that IEI supports by looking in the `KnownGPU` Enum located in `code/common/systems/known_hardware.py`.
...
```
See documentation/calibration.md for an explanation on how calibration is used for IEI's submission.
