...
class MergedEmbeddingBag(nn.Module):
    ...
    def forward(self, index: List[torch.Tensor], offset: List[torch.Tensor]) -> torch.Tensor:
        if self.int8:
            return torch.ops.torch_ipex.merged_emb_with_cat(self.weights, index, self._multi_hot)
        elif self.bf16:
            include_last = [True for i in range(26)]
            pooled_embeddings: List[torch.Tensor] = []
            res = [e(i.long(), o.long()) for (e, i, o) in zip(self.embedding_bags, index, offset)]
            B = offset[0].numel() - 1
            data = torch.cat(res, dim=1).reshape(B, self._num_embeddings, self._embedding_dim)
            return data
...
class InteractionDCNArch(nn.Module):
    ...
    def forward(
        ...
        # size B X (F*D + D)
        return self.crossnet(combined_values.reshape([B, self.ID]))
...
class DLRMMLPerf(nn.Module):
    ...
    def forward(self, densex, index, offset) -> torch.Tensor():
        ...
        # over_time = time.time() - over_start
        # print(f"sparse {dense_time*1000.0} dense {sparse_time*1000.0} inter {inter_time*1000.0} over {over_time*1000.0}")
        return out
