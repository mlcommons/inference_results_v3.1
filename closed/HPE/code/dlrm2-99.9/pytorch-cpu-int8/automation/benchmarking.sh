...
case "${model}" in
    ...
    ;;
    "dlrm2-99.9")
        cd /opt/workdir/code/dlrm2-99.9/pytorch-cpu-${type}
        CUR_DIR=`pwd`
        OUTPUT_DIR=${CUR_DIR}
    ;;   
esac
...
if [ "${model}" == "" ]
    then
        read -p "which model?(resnet50,retinanet,rnnt,3d-unet-99.9,bert-99,dlrm-99.9,gptj-99,dlrm2-99.9):" model
fi
...
then
    ...
    if [ ${offline} == "True" ]
    then
        bash /data/mlperf_data/run_clean.sh
        sleep 120 
