...
class Consumer(multiprocessing.Process):
        ...
        with torch.no_grad():
            for s in range(self.args.max_batchsize, self.args.max_batchsize + 800, 100):
                ...
                for i, h in enumerate(self.multi_hot):
                    batch_lS_i.append(torch.ones((s * h), dtype=torch.int))
                    batch_lS_o.append(torch.arange(0, (s + 1) * h, h, dtype=torch.int))
                with torch.no_grad():
    ...
    def get_samples(self, id_list):
        ...
        for i in ls:
            if self.args.use_bf16:
                X_ls.append(i.dense_features.to(torch.bfloat16))
            else:
                X_ls.append(i.dense_features)
            sparse = i.sparse_features
            features = [sparse[f'cat_{i}'] for i in range(26)]
            lsi_ls.append([f.values() for f in features])
            lso_ls.append([f.offsets() for f in features])
            lbl_ls.append(i.labels)
        X = torch.cat(X_ls)
        batch = X.shape[0]
        lS_i = []
        lS_o = []
            ...
            with torch.autograd.profiler.profile(args.enable_profiling) as prof:
                while True:
                    ...
                    try:
                        ...
                        results = self.model_predict(batch_dense_X, batch_lS_i, batch_lS_o)
    ...
    def run(self):
        ...
        self.model = backend.load(self.args, ds)
        print ('Start warmup.')
