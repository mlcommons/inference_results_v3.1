...
Our submission implements several inference harnesses stored under closed/ConnectTechInc/code/harness:
...
Our Jetson submission also uses Docker and nvidia-docker to setup the environment. However, there are extra necessary preparations to make on Jetson, including flashing the board, setting the board mode and some other hardware related setups. **Please follow the setup steps described in closed/ConnectTechInc/[README_Jetson.md](http://README_Jetson.md)**
...
- The docker container does not copy the files, and instead **mounts** the working directory (closed/ConnectTechInc) under /work in the container. This means you can edit files outside the container, and the changes will be reflected inside as well.
