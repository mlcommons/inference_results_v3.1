...
with ScopedRestrictedImport(_new_path):
    ...
    # TODO: MLPINF-2262 Remove after gptj loadgen and checkers are finalized
    benchmark_qsl_size_map["gptj"] = 13368
...
class BaseBenchmarkHarness:
    ...
    def __init__(self, args, benchmark):
        ...
        self.tensor_parallelism = dict_get(args, "tensor_parallelism", default=None)
    ...
    def _construct_terminal_command(self, argstr):
        if self.tensor_parallelism and self.tensor_parallelism > 1:
            cmd = f"mpirun -np {self.tensor_parallelism} {self.executable} {argstr}"
        else:
    ...
    def run_harness(self, flag_dict=None, skip_generate_measurements=False):
        ...
        # Special DLRM thing
        if self.benchmark in [Benchmark.DLRM, Benchmark.DLRMv2] \
