...
def generate_readme(readme_path, system_name, short_benchmark_name, scenario):
    ...
    if "Triton_CPU" in system_name:
        ...
        To run this benchmark, first follow the setup steps in `closed/ConnectTechInc/README_Triton_CPU.md`. Then to run the harness:
        ...
        For more details, please refer to `closed/ConnectTechInc/README_Triton_CPU.md`.""".format(
        ...
        For more details, please refer to `closed/ConnectTechInc/README.md`.""".format(
            benchmark=short_benchmark_name,
            scenario=scenario.valstr()))

    if "HeteroMultiUse" in system_name:
        readme_str = textwrap.dedent("""\
        To run this benchmark, first follow the setup steps in `closed/ConnectTechInc/README.md`. Then to generate the TensorRT
        engines and run the harness, first read the **Using Multiple MIG slices** section in `closed/ConnectTechInc/README.md`.
        Then follow the instructions in `closed/ConnectTechInc/documentation/heterogeneous_mig.md` to run benchmarks.""")
...
def generate_calibration_process(system_name, calibration_process_path, short_benchmark_name, scenario):
    if "Triton_CPU" in system_name:
        ...
        To calibrate this benchmark, please follow the steps in `closed/ConnectTechInc/calibration_triton_cpu/OpenVINO/{benchmark}/README.md`.""".format(
            benchmark=short_benchmark_name))
    else:
        calibration_process_str = textwrap.dedent("""\
...
def generate_powersetting_adoc(powersetting_path):
    ...
    section of the README.md located in closed/ConnectTechInc.
    """)
...
def generate_analyzer_table(analyzer_table_path, system_name):
    system_name_to_power_cfg_map = {
        ...
        # Systems for v3.1
        "H100-PCIe-80GBx8_TRT_MaxQ": "ipp1-1470",
