    ...
    if (!(externalContiguity && internalContiguity))
    {
        if (!mNumericInputBuf.isDefaultHostPtr())
        {
            ...
            mCategoricalInputBuf.H2H(
                static_cast<DLRMv2CategoricalInputType*>(qsl->GetSampleAddress(mTasks[0].querySample.index, 1)),
                pairsOffset, 0);
        }
        ...
        if (internalContiguity)
        {
            ...
            mCategoricalInputBuf.H2H(
                static_cast<DLRMv2CategoricalInputType*>(qsl->GetSampleAddress(firstTask.querySample.index, 1)),
...
void Batch::IndividualH2H(size_t tasksOffset, size_t pairsOffset, size_t pairsToCopy)
{
    ...
    while (copiedPairs < pairsToCopy)
    {
        ...
        mNumericInputBuf.H2H(static_cast<DLRMv2NumericInputType*>(qsl->GetSampleAddress(qslIdx, 0)), numIndividualPairs,
            pairsOffset + copiedPairs);
        mCategoricalInputBuf.H2H(static_cast<DLRMv2CategoricalInputType*>(qsl->GetSampleAddress(qslIdx, 1)),
...
BatchMaker::BatchMaker(size_t numStagingThreads, size_t numStagingBatches, size_t maxBatchSize,
    ...
    std::shared_ptr<DLRMv2SampleLibrary> qsl, int64_t numaIdx, int64_t numaNum, std::vector<int> cpus)
    : mMaxBatchSize(maxBatchSize)
