...
def generate_readme(readme_path, system_name, short_benchmark_name, scenario):
    ...
    if "Triton_CPU" in system_name:
        ...
        To run this benchmark, first follow the setup steps in `closed/xFusion/README_Triton_CPU.md`. Then to run the harness:
        ...
        For more details, please refer to `closed/xFusion/README_Triton_CPU.md`.""".format(
        ...
        For more details, please refer to `closed/xFusion/README.md`.""".format(
            benchmark=short_benchmark_name,
            scenario=scenario.valstr()))

    if "HeteroMultiUse" in system_name:
        readme_str = textwrap.dedent("""\
        To run this benchmark, first follow the setup steps in `closed/xFusion/README.md`. Then to generate the TensorRT
        engines and run the harness, first read the **Using Multiple MIG slices** section in `closed/xFusion/README.md`.
        Then follow the instructions in `closed/xFusion/documentation/heterogeneous_mig.md` to run benchmarks.""")
...
def generate_calibration_process(system_name, calibration_process_path, short_benchmark_name, scenario):
    if "Triton_CPU" in system_name:
        ...
        To calibrate this benchmark, please follow the steps in `closed/xFusion/calibration_triton_cpu/OpenVINO/{benchmark}/README.md`.""".format(
            benchmark=short_benchmark_name))
    else:
        calibration_process_str = textwrap.dedent("""\
...
def generate_powersetting_adoc(powersetting_path):
    ...
    section of the README.md located in closed/xFusion.
    """)
