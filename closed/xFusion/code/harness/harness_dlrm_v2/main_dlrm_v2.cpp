...
DEFINE_bool(use_batcher_thread_per_device, true, "Instantiate BatchMaker per GPU if server_num_issue_query_threads is not used");
...
int main(int argc, char* argv[])
{
    ...
    // Scope to force all smart objects destruction before CUDA context resets
    {
        ...
        if (!QSLNumaConfig.empty())
        {
            ...
            const int32_t numQSL = QSLNumaConfig.size();
            for (int32_t qslIdx = 0; qslIdx < numQSL; qslIdx++)
            {
                ...
                auto constructQsl = [&]()
                {
                    ...
                    int32_t splitIdx = 0;
                    for (auto& numaIdx: QSLNumaConfig[qslIdx])
                    {
                        auto myDevs = numaConfig[numaIdx].first;
                    ...
                    auto oneQsl = std::make_shared<DLRMv2SampleLibrary>("DLRMv2 QSL", FLAGS_map_path,
        ...
        else if (!numaConfig.empty())
        {
            ...
            for (int32_t numaIdx = 0; numaIdx < nbNumas; numaIdx++)
            {
                ...
                auto constructQsl = [&]()
                {
                    ...
                    std::vector<int32_t> numaIndices;
                    std::map<int32_t, int32_t> QSLDevMap;
                    QSLDevMap[0] = 0;
                    numaIndices.emplace_back(numaIdx);
                    std::this_thread::sleep_for(std::chrono::milliseconds(100));
                    auto oneQsl = std::make_shared<DLRMv2SampleLibrary>("DLRMv2 QSL", FLAGS_map_path,
                        splitString(FLAGS_tensor_path, ","), originalPartition, FLAGS_performance_sample_count,
        ...
        auto dlrm_v2_server = std::make_shared<DLRMv2Server>("DLRMv2 SERVER", FLAGS_gpu_engines, qsls, gpus,
            FLAGS_gpu_batch_size, FLAGS_gpu_num_bundles, FLAGS_complete_threads, FLAGS_gpu_inference_streams,
            FLAGS_warmup_duration, FLAGS_num_staging_threads, FLAGS_num_staging_batches,
            FLAGS_max_pairs_per_staging_thread, FLAGS_check_contiguity, FLAGS_start_from_device, numaConfig,
            FLAGS_server_num_issue_query_threads, FLAGS_use_batcher_thread_per_device, FLAGS_eviction_last, numaToQslMap, 
            FLAGS_verbose_nvtx);

        LOG(INFO) << "Starting running actual test.";
        cudaProfilerStart();
        std::shared_ptr<DLRMv2SampleLibraryEnsemble> qslEnsemble(new DLRMv2SampleLibraryEnsemble(qsls));
        StartTest(dlrm_v2_server.get(), qslEnsemble.get(), testSettings, logSettings);
        cudaProfilerStop();
