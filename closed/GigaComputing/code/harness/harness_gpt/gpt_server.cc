...
std::vector<std::pair<int32_t, int32_t>> GPTServer::sortSamples(std::vector<mlperf::QuerySample> const& samples) const
{
    ...
    if (mEnableSort)
    {
        ...
        size_t totalLen = sequenceSamplePosAndLength.size();
        // Sort the waiting queries by segments when the queueing queries are larger than 20.
        size_t segment = (totalLen > 20) ? mNumSortSegments : 1;
        size_t segLen = totalLen / segment;
        for (size_t i = 0; i < segment; ++i)
        {
            size_t start_idx = i * segLen;
            size_t end_idx = (i == segment - 1) ? totalLen : (segLen * (i+1));
            std::sort(sequenceSamplePosAndLength.begin() + start_idx, sequenceSamplePosAndLength.begin() + end_idx,
...
GPTServer::GPTServer(std::string const& name, std::shared_ptr<qsl::SampleLibrary> qsl, std::string const enginePath,
    ...
    bool verboseNVTX, bool useFp8, bool isMain, bool isGPTJ, bool enableSort, uint32_t numSortSegments)
    : mName(name)
    , mQsl(qsl)
    , mStopGetTasks(false)
    ...
    , mNumSortSegments(numSortSegments)
{
    {
