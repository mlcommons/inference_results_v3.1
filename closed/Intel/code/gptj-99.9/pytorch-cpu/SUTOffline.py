    ...
    def handleTasks(self, i, task_queue, result_queue, pid, start_core, num_cores):
        ...
        while True:
            try:
                ...
                t1 = time.time()
                if DEBUG_PRINT:
                    print(f"P{self.proc_idx}-{i}: time: {t1-t0:6.2f} input_ids = {list(input_ids.shape)} output = {list(output.shape)} out_len: {output.shape[1] - input_ids.shape[1]} t0= {t0-init_time:.2f} ids = {sample_index_list}")
