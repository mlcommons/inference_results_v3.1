...
def getArgs():
    ...
    parser.add_argument("--precision", choices=["int8", "bf16", "fp32", "mix"], help="Model precision to run", default="fp32")
    ...
    parser.add_argument("--max-dynamic-batch-size", type=int, help="Max dynamic batch size for server mode", default=2)
    parser.add_argument("--hp-threshold", type=int, help="Threshold for high priority sequence length", default=1600)
    parser.add_argument("--use-dynamic-batching", action="store_true", help="Whether to use dynamic batching in server mode")
    parser.add_argument("--numa-offset", type=int, help="NUMA offset for using HBM memory in Server mode. Set it to 2 when using 2S SPR-HBM in QUAD mode", default=0)
