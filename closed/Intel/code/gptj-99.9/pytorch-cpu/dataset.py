...
class Dataset(object):
    ...
    def getWarmupSamples(self):
        ...
        warmup_samples = []
        len_list = []
        len_list += list(range(64, 128, 1))
        len_list += list(range(256, 320, 1))
        len_list += list(range(128, 1919, 64))
        for i in len_list:
            input_ids = torch.randint(100, 50000, [1, i])
            attn_mask = torch.ones([1, i])
            warmup_samples.append((input_ids, i, attn_mask))
    ...
    def postProcess(self, query_id_list, sample_index_list, out_tokens, input_seq_lens):
        ...
        for i in range(out_tokens.shape[0]):
            ...
            #print(f"XXX: {sample_index} : {input_len} : {pred.shape[0]} : {pred.tolist()}")
            preds.append(pred)
