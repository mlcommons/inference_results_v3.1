...
if [ ${model} == "resnet50" ] || [ ${model} == "retinanet" ] || [ ${model} == "rnnt" ] || [ ${model} == "3d-unet-99.9" ] || [ ${model} == "bert-99" ];then
        read -p "Data type?(int8):" type
        if [ ${type} != "int8" ];then
                echo "Error: Invalid data type. Please choose 'int8'."
                exit 1
        fi
fi
...
if [ ${Skip_docker_build} == "False" ];then
 	...
 	if [ ${model} == "resnet50" ] || [ ${model} == "retinanet" ] || [ ${model} == "rnnt" ] || [ ${model} == "3d-unet-99.9" ] || [ ${model} == "bert-99" ];then
        	bash ./build_*${model}_container.sh >build.log
	else
 		bash ./build_*${model}_*${type}_container.sh >build.log
	fi
	...
	["dlrm-v2-99-int8"]="mlperf_inference_dlrm2"
