...
DEFINE_bool(enable_sort, true, "Sort QSL samples before inference");
DEFINE_uint32(num_sort_segments, 2, "Number of segments to divide before sorting.");
...
int main(int argc, char** argv)
{
    ...
    // Scope to force all smart objects destruction before CUDA context resets
    {
        ...
        auto sut = std::make_shared<GPTServer>(sutName, qsl, parseEngineFromRank(FLAGS_gpu_engines, mpiRank, isGPTJ),
            ...
            FLAGS_use_fp8, isMain, isGPTJ, FLAGS_enable_sort, FLAGS_num_sort_segments);
