...
BaseSUT::BaseSUT(
    ...
    const std::string& profiler_folder, int profiler_iter, int warmup_iter, int num_numa)
    : qsl_(sample_file),
      model_(model_file, num_numa),
  ...
  nCoresPerSocket_ = nMaxProc_ / nSockets_ / (mHt_ + 1);
  ...
  std::cout << "Numa nodes: " << nSockets_ << std::endl;
...
OfflineSUT::OfflineSUT(
  ...
  //Construct instances & bind core
  int part[MAX_NUM_NUMA];
  for(int i = 0; i < nSockets_ && i < MAX_NUM_NUMA; i++){
      part[i] = nMaxThread_ - nCoresPerSocket_ * i;
  }
  ...
  std::generate(
      ...
      [=, index = 0, root = 0]() mutable {
        root = part[index & (nSockets_ - 1)] - nThreadsPerInstance_ * ((index >> (int)log2(nSockets_)) + 1);
        ++index;
        return root;
      });
...
ServerSUT::ServerSUT(
  ...
  if (ceil(nProducers_ / nSockets_) * nThreadsPerProducer_ +
    ...
    std::cout << "Run with staggered binding.\n" << std::endl; 

    int producer_part[MAX_NUM_NUMA];
    for(int i = 0; i < nSockets_ && i < MAX_NUM_NUMA; i++){
        producer_part[i] = nMaxThread_ - nCoresPerSocket_ * (nSockets_ - i); 
    }
    std::generate(
        producer_root.begin(), producer_root.end(),
        [=, index = 0, root = 0]() mutable {
          root = producer_part[index & (nSockets_ - 1)] + nThreadsPerProducer_ * (index >> (int)log2(nSockets_));
          ++index;
          return root;
        });

    int consumer_part[MAX_NUM_NUMA];
    for(int i = 0; i < nSockets_; i++){
        consumer_part[i] = nMaxThread_ - ( nProducers_ / nSockets_ ) * i * nThreadsPerProducer_ - ( nConsumers_ / nSockets_) * i * nThreadsPerConsumer_;
    }
    
    std::generate(
        consumer_root.begin(), consumer_root.end(),
        [=, index = 0, root = 0]() mutable {
          root = consumer_part[index & (nSockets_ - 1)] -
                 nThreadsPerConsumer_ * ((index >> (int)log2(nSockets_)) + 1);
          ++index;
          return root;
        });
  } else {  // sequential binding
	    
    std::cout << "Run with sequential binding.\n" << std::endl; 
...
void ServerSUT::thProducer(int index, int root) {
  ...
  for (int i = 0; i < nThreadsPerProducer_; ++i) { 
     places[i] = (root + i);
     std::cout << "#producer index " << index << " which " << which << " places[" << i << "] = " << places[i] << "\n";
  }
