...
Our submission implements several inference harnesses stored under closed/Dell/code/harness:
...
Benchmarks are stored in `closed/Dell/code`. Each benchmark, as per MLPerf Inference requirements, contains a `README.md` detailing instructions and documentation for that benchmark. **However**, as a rule of thumb, **follow this guide first** from start to finish before moving on to benchmark-specific `README`s, as this guide has many wrapper commands to automate the same steps across multiple benchmarks at the same time.
...
- [nvidia-docker](https://github.com/NVIDIA/nvidia-docker)
    - libnvidia-container >= 1.4.0
- **Additional setup steps described in closed/Dell/[README_Jetson.md](http://README_Jetson.md)**
...
Enter the container by entering the `closed/Dell` directory and running:
...
- The docker container does not copy the files, and instead **mounts** the working directory (closed/Dell) under /work in the container. This means you can edit files outside the container, and the changes will be reflected inside as well.
...
1. Set the machine to the desired power mode. The Makefile in closed/Dell contains a target called `power_set_maxq_state` that describes the power settings we use for our MaxQ submissions. Run this make target before proceeding.
2. Set up a Windows power director machine with the following requirements.
    ...
    4. There exists an administrator user `lab` with password `labuser`. If your administrator account has different login information, set `POWER_SERVER_USERNAME` and `POWER_SERVER_PASSWORD` in `closed/Dell/Makefile` to the correct credentials.
