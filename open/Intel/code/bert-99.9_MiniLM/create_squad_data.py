...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        text  = ' '
        text = text.join(example.doc_tokens)
        ...
        offset_mapping.append(offset_mapping[0])
        all_tokens = tokenizer.tokenize(text)
        j = 0
        orig_to_tok_index.append(len(all_doc_tokens))
        for (i, word) in enumerate(example.doc_tokens):
            start = offset_mapping[j][0]
            end = offset_mapping[j][1]
            all_doc_tokens.append(all_tokens[j])
            tok_to_orig_index.append(i)
            while word != text[start:end] or offset_mapping[j]==offset_mapping[j+1]:
                j = j+1
                end = offset_mapping[j][1]
                tok_to_orig_index.append(i)
                all_doc_tokens.append(all_tokens[j])
