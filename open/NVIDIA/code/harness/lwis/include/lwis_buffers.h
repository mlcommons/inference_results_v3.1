...
public:
        ...
        , mProfileIndices(profileIndices)
    {
        ...
        for (size_t engineIdx = 0; engineIdx < engines.size(); ++engineIdx)
        {
            ...
            {
                std::string bindingName{engine->getBindingName(bindingIdx)};
                // Create host and device buffers
                bool isInput = engine->bindingIsInput(bindingIdx);

                size_t vol = lwis::volume(engine->getBindingDimensions(bindingIdx),
                    engine->getBindingFormat(bindingIdx), engine->hasImplicitBatchDimension());
                ...
                if (engineIdx >= 1 && isInput)
                {
                    ...
                    std::cout << "Engine-" << engineIdx << " reusing address from Engine-" << engineIdx - 1
                              << ": host address-" << mManagedBuffers[engineIdx - 1].at(bindingName)->hostBuffer.data()
                              << " device address-"
                              << mManagedBuffers[engineIdx - 1].at(bindingName)->deviceBuffer.data() << " | ";
                    std::cout << (isInput ? "Input" : "Output") << " BindingName: " << bindingName
                              << " BindingSize: " << bindingSize << std::endl;
#endif
                    for (size_t loop = 0; loop < batchLoopCount; ++loop)
                    {
                        uint8_t* packedHostBinding
                            = static_cast<uint8_t*>(mManagedBuffers[engineIdx - 1].at(bindingName)->hostBuffer.data())
                            + bindingSize * loop;
                        uint8_t* packedDeviceBinding
                            = static_cast<uint8_t*>(mManagedBuffers[engineIdx - 1].at(bindingName)->deviceBuffer.data())
                            + bindingSize * loop;
                ...
                else
                {
                    ...
                    std::cout << (isInput ? "Input" : "Output") << " BindingName: " << bindingName
                              << " BindingSize: " << bindingSize << " AllocationSize: " << allocationSize << std::endl;
#endif
                    for (size_t loop = 0; loop < batchLoopCount; ++loop)
                    {
                    ...
                    mManagedBuffers[engineIdx].emplace(bindingName, std::move(manBuf));
                }
            }
#if DEBUG_LWIS_BUFFER
    ...
    size_t volume(size_t const engineIdx, std::string const& tensorName) const
    {
        ...
        return lwis::volume(mEngines[engineIdx]->getBindingDimensions(index),
            mEngines[engineIdx]->getBindingFormat(index), mEngines[engineIdx]->hasImplicitBatchDimension());
    }
    ...
    //!
    void copyInputToDeviceAsync(size_t const engineIdx, size_t const bindingIdx, cudaStream_t const& stream = 0,
        void* src = nullptr, size_t const size = 0, size_t const offset = 0)
    {
    ...
    //!
    void copyOutputToHostAsync(size_t const engineIdx, size_t const bindingIdx, cudaStream_t const& stream = 0,
        void* dst = nullptr, size_t const size = 0, size_t const offset = 0)
    {
...
private:
    ...
    void* getBuffer(size_t const engineIdx, bool const isHost, size_t const bindingIdx) const
    {
        CHECK(mEngines[engineIdx]->getBindingName(bindingIdx) != nullptr);
        std::string tensorName{mEngines[engineIdx]->getBindingName(bindingIdx)};
        return (isHost ? mManagedBuffers[engineIdx].at(tensorName)->hostBuffer.data()
                       : mManagedBuffers[engineIdx].at(tensorName)->deviceBuffer.data());
    }

    void memcpyBuffers(size_t const engineIdx, size_t const bindingIdx, bool const copyInput, bool const deviceToHost,
        bool const async, cudaStream_t const& stream = 0, void* buf = nullptr, size_t const size = 0,
        size_t const offset = 0)
    {
        CHECK(engineIdx < mEngines.size()) << "Invalid engine index.";
        CHECK((copyInput && mEngines[engineIdx]->bindingIsInput(bindingIdx))
            || (!copyInput && !mEngines[engineIdx]->bindingIsInput(bindingIdx)))
        ...
        CHECK(mManagedBuffers[engineIdx].find(tensorName) != mManagedBuffers[engineIdx].end())
            << "Invalid bindingIdx: " << bindingIdx << " numBufferBindings: " << numBufferBindings
