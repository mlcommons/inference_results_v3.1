        ...
        for (auto const& deviceTypes : params.EngineNames)
        {
                 ...
                 deviceInstance++)
            {
                ...
                auto device = std::make_shared<lwis::Device>(deviceNum, numCopyStreams, numInferStreams,
                    ...
                    m_ServerSettings.elRatio, m_ServerSettings.VerboseNVTX);
                ...
                for (size_t engineIdx = 0; engineIdx < deviceTypes.size(); ++engineIdx)
                {
                    ...
                    size_t const batchSize = isDLADevice ? m_ServerSettings.DLABatchSizes[engineIdx]
                                                         : m_ServerSettings.GPUBatchSizes[engineIdx];
                    size_t const batchLoopCount = isDLADevice ? m_ServerSettings.DLALoopCounts[engineIdx]
                                                              : m_ServerSettings.GPULoopCounts[engineIdx];
...
std::vector<std::vector<EngineBindings>> Server::CopySamples(DevicePtr_t device, size_t batchSizeTotal,
    std::deque<mlperf::QuerySample>::iterator begin, std::deque<mlperf::QuerySample>::iterator end,
    cudaStream_t copyStream, bool directHostAccess, bool staging)
{
