...
struct BERTModelBase
{
    ...
    virtual void setInputShapes(
        std::shared_ptr<nvinfer1::IExecutionContext> context, int sumS, int B, int maxS) const = 0;
...
struct BERTModel : BERTModelBase
{
    ...
    virtual void setInputShapes(
        std::shared_ptr<nvinfer1::IExecutionContext> context, int sumS, int B, int maxS) const override;
...
public:
    BERTCoreVS(const std::vector<std::shared_ptr<nvinfer1::ICudaEngine>>& engines, int numCopyStreams,
        int numDupProfiles, int profileIdx, bool useGraphs, int deviceId, bool useFp8, double elRatio,
        bool verboseNVTX);
    ~BERTCoreVS();
