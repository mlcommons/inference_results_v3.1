...
static void createModelStreams(
    const std::string& enginePath, std::shared_ptr<std::vector<std::vector<char>>> trtModelStreams)
{
...
void BERTServer::CreateEnginesPerGPU(int deviceId, std::shared_ptr<nvinfer1::IRuntime> runtime,
    std::shared_ptr<std::mutex> pMtx, const std::shared_ptr<std::vector<std::vector<char>>> trtModelStreams)
{
    CHECK_EQ(cudaSetDevice(deviceId), cudaSuccess);
...
{
    {
        ...
        {
            engineCreationThreads.emplace_back(
                &BERTServer::CreateEnginesPerGPU, this, deviceId, mRuntime, pMtx, trtModelStreams);
        }
