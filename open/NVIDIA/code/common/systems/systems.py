...
class SystemConfiguration(Hardware):
    ...
    def matches(self, other) -> bool:
        if other.__class__ == self.__class__:
            c = self.host_cpu_conf == other.host_cpu_conf
            h = self.host_mem_conf == other.host_mem_conf
            a = self.accelerator_conf == other.accelerator_conf
            if not c:
                return False
            if not h:
                return False
    ...
    def find_system_json_paths(self) -> List[str]:
        ...
        harness_list = [HarnessType.Custom]
