...
auto GPTJDataProcess = [](moffett::spu_backend::InOutBindingHandle input_binding, const std::vector<std::size_t>& real_input_size,
...
auto GPTJResponseCallback = [](moffett::spu_backend::Batch* batch, moffett::spu_backend::InOutBindingHandle infer_result_data, const std::vector<std::size_t>& output_data_size,
  ...
  for (int batch_idx = 0; batch_idx < size; ++batch_idx) {
    int64_t* ptr = reinterpret_cast<int64_t*>((*infer_result_data)[0][batch_idx]);
    size_t index = 0;
    while (ptr[index] != 50256 && index < 128) {
      index++;
    }
    (*responses)[batch_idx].data = (uintptr_t)ptr;
    (*responses)[batch_idx].size = index * sizeof(int64_t);
  }
...
void inference() {
  ...
  moffett::spu_backend::ServerPtr_t server = std::make_shared<moffett::spu_backend::Server>("GPTJ");
  ...
  server->Setup(backendSettings, sut_params, GPTJResponseCallback,
                GPTJDataProcess); // Pass the requested server settings and params to our SUT
